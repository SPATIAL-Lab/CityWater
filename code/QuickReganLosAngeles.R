...
vectv <- mask(vectv, subset(cities, cities$NAME10 == "Los Angeles--Long Beach--Anaheim, CA"))
ggplot() + 
  geom_sf(data = terra::subset(cities, cities$NAME10 == "Los Angeles--Long Beach--Anaheim, CA")) + 
  geom_sf(data = terra::subset(vectv, vectv$cluster_location == "Los Angeles"), 
          aes(fill = cluster_number), size = 4, shape = 21) +   
  scale_fill_manual(values = c("#003f5c", "#d2042d", "#ffa600")) +
  theme_void() +
  theme(legend.position = "none")

ggplot() + 
  geom_point(data = subset(vign, cluster_location == "Los Angeles"), 
             aes(x = d18O, y = d2H, color = cluster_number), size = 3) + 
  scale_color_manual(values = c("#003f5c", "#d2042d", "#ffa600")) +
  theme_classic() + 
  theme(legend.position = 'none')
