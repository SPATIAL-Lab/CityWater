...
# Fixing bins -------------------------------------------------------------
tapData.sf_Atl_7
ggplot(tapData_1_ridge, aes(x = d2H, y = Cluster_Location, fill = ..x..)) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01)
ggplot(tapData.sf_Atl_7, aes(x = d2H, y = Cluster_Location, fill = ..x..)) +
  geom_density_ridges_gradient()  

ggplot(tapData.sf_Atl_7, aes(d_ex)) + 
  geom_density(
  )
ggplot(tapData.sf_Atl_7, aes(d2H)) + 
  geom_density(
  )


ggplot(tapData_1_ridge, 
       aes(x = d2H, y = Cluster_Location, fill = ..x..)) + 
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
  scale_fill_gradient(low = "#003f5c", high = "#84edff", na.value = NA) +
  theme_bw(base_size = 16) +
  theme(
  ...
  labs(
    x = expression(paste(delta^2, "H", " (\u2030, VSMOW)"))
  ) 


ggplot() + 
  geom_density_ridges_gradient(tapData.sf_Atl_7,  aes(x = d2H, fill = ..x..)) + 
  geom_density_ridges_gradient(tapData.sf_DF_6,  aes(x = d2H, fill = ..x..))
...
ggplot(density, aes(x = d2H, y = Cluster_Location)) + geom_density_ridges()
