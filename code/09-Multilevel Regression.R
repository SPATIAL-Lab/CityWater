...
ughHio <- multivariate <- read.csv("data/multivariate.csv") %>% 
  filter(cluster_location != "Oahu" & cluster_location != "Hawaii" & 
           cluster_location != "Youngstown" & cluster_location != "Cleveland-Akron")
ughHio <- left_join(ughHio, datasummary, by = 'cluster_location') %>% 
  rename('sd' = 'd18O_sd', 
         'idr' = 'IDR_O')
...
model$water_use = ughHio$water_use / ughHio$pop * 1e6 * 3.78
model$streamflow = log(model$streamflow)
model$water_use = log(model$water_use)
model$popdensity = log(model$popdensity)
...
png("figures/Fig4Nohio.png", width = 8.2, height = 9.2, units = "in", res = 600)
...
nohio <- lm(sqrt(idr) ~ streamflow + lon + medincome,
                 data = model)

summary(nohio)
plot(density(nohio$residuals))
shapiro.test(nohio$residuals)
plot(nohio)
...
# Without YNG -----------------------------------------------------
noYNG <- multivariate <- read.csv("data/multivariate.csv") %>% 
  filter(cluster_location != "Oahu" & cluster_location != "Hawaii" & 
           cluster_location != "Youngstown")
noYNG <- left_join(noYNG, datasummary, by = 'cluster_location') %>% 
  rename('sd' = 'd18O_sd', 
         'idr' = 'IDR_O')
...
model$water_use = noYNG$water_use / noYNG$pop * 1e6 * 3.78
model$streamflow = log(model$streamflow)
model$water_use = log(model$water_use)
model$popdensity = log(model$popdensity)
...
png("figures/Fig4NoYNG.png", width = 8.2, height = 9.2, units = "in", res = 600)
...
# best model, 3 covariates:
noYNGmodel <- lm(sqrt(idr) ~ streamflow + lon + ruggedness + medincome + water_use,
            data = model)

summary(noYNGmodel)
plot(density(noYNGmodel$residuals))
shapiro.test(noYNGmodel$residuals)
plot(noYNGmodel)
...
# No Cleveland?! ----------------------------------------------------------
noCLE <- multivariate <- read.csv("data/multivariate.csv") %>% 
  filter(cluster_location != "Oahu" & cluster_location != "Hawaii" & 
           cluster_location != "Cleveland-Akron")
noCLE <- left_join(noCLE, datasummary, by = 'cluster_location') %>% 
  rename('sd' = 'd18O_sd', 
         'idr' = 'IDR_O')
...
model$water_use = noCLE$water_use / noCLE$pop * 1e6 * 3.78
model$streamflow = log(model$streamflow)
model$water_use = log(model$water_use)
model$popdensity = log(model$popdensity)
...
# best model, 3 covariates:
noCLEmodel <- lm(sqrt(idr) ~ streamflow + lon + ruggedness,
                 data = model)

summary(noCLEmodel)
plot(density(noCLEmodel$residuals))
shapiro.test(noCLEmodel$residuals)
plot(noCLEmodel)
